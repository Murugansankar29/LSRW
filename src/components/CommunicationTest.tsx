...
import { Mic, Square, Clock, FileText, Volume2 } from 'lucide-react';
...
export const CommunicationTest: React.FC<CommunicationTestProps> = ({ prompts, onComplete }) => {
  ...
  const [autoStartIntro, setAutoStartIntro] = useState(false);
  ...
  const [autoStartSpeaking, setAutoStartSpeaking] = useState(false);
  ...
  useEffect(() => {
    ...
    if (introPrepTime > 0) {
      const prepTimer = setInterval(() => {
        setIntroPrepTime((prev) => {
          if (prev === 1) setAutoStartIntro(true);
          return prev > 0 ? prev - 1 : 0;
        });
      }, 1000);
      return () => clearInterval(prepTimer);
    }
    if (autoStartIntro && !isRecording && introPrepTime === 0 && introSpeakingTime === 60) {
      setAutoStartIntro(false);
  ...
  }, [currentStep, introPrepTime, introSpeakingTime, isRecording, autoStartIntro]);

  // Speaking test timers
  useEffect(() => {
    if (currentStep !== 1) return;
    
    // Reset auto-start flag when entering speaking test or changing sentence
    if (speakingPrepTime === 5) {
      setAutoStartSpeaking(false);
    }
    ...
    if (speakingPrepTime > 0) {
      const prepTimer = setInterval(() => {
        setSpeakingPrepTime((prev) => {
          if (prev === 1) {
            console.log('Setting autoStartSpeaking to true');
            setAutoStartSpeaking(true);
          }
          return prev > 0 ? prev - 1 : 0;
        });
      }, 1000);
  ...
  // Separate effect for auto-starting speaking recording
  useEffect(() => {
    const autoStart = async () => {
      if (currentStep === 1 && autoStartSpeaking && !isRecording && speakingPrepTime === 0 && speakingRecordTime === 15) {
        console.log('Starting speaking recording for sentence:', currentSpeakingIndex + 1);
        setAutoStartSpeaking(false);
        ...
        try {
          ...
          setLiveSpeakingTranscripts(prev => {
            const newTranscripts = [...prev];
            newTranscripts[currentSpeakingIndex] = '';
          ...
          await speechService.startLiveRecording((transcript: string, isFinal: boolean) => {
            ...
            setLiveSpeakingTranscripts(prev => {
              ...
              updatedTranscripts[currentSpeakingIndex] = transcript;
    ...
    autoStart();
  }, [currentStep, autoStartSpeaking, isRecording, speakingPrepTime, speakingRecordTime, currentSpeakingIndex]);
  ...
  useEffect(() => {
    ...
    if (audioPlayed && autoStartListening && !isRecording && listeningRecordTime === 15) {
  ...
  }, [currentStep, audioPlayed, listeningRecordTime, isRecording, autoStartListening]);
  ...
  const handleComplete = () => {
    ...
    result.speaking.sentences = prompts.speakingSentences;
          ...
          <>
            <div className="mb-6">
              <motion.button
                ...
                disabled={!speechService.isRecognitionSupported() || isRecording}
                className={`w-20 h-20 rounded-full flex items-center justify-center transition-all duration-200 ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-yellow-500 hover:bg-yellow-600'} ${!speechService.isRecognitionSupported() ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {isRecording ? <Square className="w-10 h-10 text-white" /> : <Mic className="w-10 h-10 text-black" />}
              </motion.button>
              <p className="text-yellow-300 mt-4">
                {isRecording ? `Recording... (${introSpeakingTime}s left)` : 'Recording will start automatically'}
              </p>
  ...
  const renderSpeakingTest = () => {
    const currentSentence = prompts.speakingSentences[currentSpeakingIndex];
    const isCompleted = currentSpeakingIndex >= prompts.speakingSentences.length;
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        <div className="text-center mb-8">
          <h3 className="text-2xl font-semibold text-yellow-400 mb-4">Speaking Test</h3>
          <p className="text-yellow-200">Read the following sentences aloud clearly and naturally.</p>
          <p className="text-yellow-300 text-sm mt-2">(5 marks)</p>
        </div>
        {!isCompleted && currentSentence && (
          <div className="bg-gray-700/50 rounded-xl p-6">
            <p className="text-lg text-yellow-200 mb-4 text-center">Sentence {currentSpeakingIndex + 1} of {prompts.speakingSentences.length}</p>
              ...
              {speakingPrepTime > 0 ? (
                ...
                <div className="space-y-2">
                  <p className="text-yellow-300 text-lg">
                    {isRecording ? `Recording... ${speakingRecordTime}s` : 'Ready to record'}
                  </p>
                  {!isRecording && speakingPrepTime === 0 && (
                    <p className="text-yellow-200 text-sm">
                      Auto-start failed? Click the microphone to start manually
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <motion.button 
                  ...
                  onClick={() => {
                    ...
                    if (currentSpeakingIndex === prompts.speakingSentences.length - 1) {
                      ...
                      setAutoStartSpeaking(false); // Reset auto-start flag
                    }
                  }} 
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg mt-3"
                >
                  {currentSpeakingIndex === prompts.speakingSentences.length - 1 ? "Next Section" : "Next Sentence"}
                </motion.button>
              </div>
            </div>
          </div>
        ...
        {isCompleted && (
          <div className="text-center text-yellow-300 bg-gray-700/50 rounded-xl p-6">
            ...
            <p>All {prompts.speakingSentences.length} sentences recorded successfully.</p>
          </div>
        )}
      </motion.div>
    );
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        {!isCompleted && currentAudio && (
          <div className="bg-gray-700/50 rounded-xl p-6">
            ...
            <div className="text-center space-y-4">
              {audioPlayed ? (
                <p className="text-yellow-300 text-lg">Recording... {listeningRecordTime}s</p>
              ) : null}
              <motion.button disabled className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-200 ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-gray-600'} opacity-50 cursor-not-allowed`}>
                {isRecording ? <Square className="w-8 h-8 text-white" /> : <Mic className="w-8 h-8 text-black" />}
              </motion.button>
              <div className="bg-gray-800 rounded-lg p-4">
                <h4 className="text-yellow-300 font-semibold mb-2">
                  {isRecording ? 'Live Transcription:' : 'You said:'}
